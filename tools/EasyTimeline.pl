...
foreach $line (@lines) {
    if ( $line =~ / font\:/ ) { $fonts_specified = $true; last; }
}
...
"\nNo external fonts specified in input ->\nTreat input as ascii and use internal Ploticus font.\n\n";
    ...
    # in extended ascii range are not available
        $line =~
tr /¡¢£¥¦©ª«¬­®¯±²³µ¶·¹º»¼½¾¿ÀÅÆÇÈÊÌÏÒÖØÛÞàåæèêìïðòøþÿ/_________________________AAACEEIIOO0Ü_aaaeeii_o0_y/;
    }
}
...
sub ParseArguments {
    ...
    &Abort("Specify input file as: -i filename")
    ...
    $articlepath = @options{
        "A"}; # For MediaWiki: Path of an article, relative to this servers root
...
sub GetFontPath {
    ...
    foreach $font ( sort keys %fonts ) { $fontlist .= "$font, "; }
    $fontlist =~ s/,$//;
...
"Command 'DrawLines' has been renamed to 'LineData', please adjust.\n"
        ...
        if (
            !(
                $name =~
                /^(?:BarData|Colors|Fonts|LineData|PlotData|TextData)$/i
            )
          )
        {
...
sub CollectAttributes {
    ...
    $line =~ s/(\stext\:[^\s\:]*)\:/$1'colon'/i
      ;    # replace colon (:), would conflict with syntax
    $line =~
      s/(https?)\:/$1'colon'/i;  # replace colon (:), would conflict with syntax
...
"BarData attribute 'link' contains implicit (wiki style) link.\n"
                ...
                if ( ( $attrvalue !~ /^\d+$/ ) || ( $attrvalue < 2 ) ) {
                    &Error(
"BarData attribute 'maxbars' invalid. Specify a integral number, larger than 1\n"
                    );
                    &GetData;
                    next BarData;
                }
...
"BarData attribute 'link' not valid in combination with attribute 'barset'.\n"
        ...
        if ( ( $barset eq "" ) && ( $maxbars ne "" ) ) {
            &Error(
"BarData attribute 'maxbars' only valid in combination with attribute 'barset'.\n"
            );
            &GetData;
            next BarData;
        }
...
"BarData attribute bar:'$bar' invalid.\nUse only characters 'a'-'z', 'A'-'Z', '0'-'9', '_'\n"
        ...
        if ( $bar ne "" ) {
            ...
            if ( $maxbars ne "" ) { @BarSetMaxBars{$barset} = $maxbars; }
        }
...
sub ParseFonts {
    ...
    if ($NoData) {
        &Error(
            "Data expected for command 'Fonts', but line is not indented.\n");
        return;
    }

  Fonts:
    while ( ( !$InputParsed ) && ( !$NoData ) ) {
        if ( !&ValidAttributes("Fonts") ) { &GetData; next; }
        ...
        foreach $attribute ( keys %Attributes ) {
            ...
            if ( $attribute =~ /Id/i ) {
                ...
                if ( !( $attrvalue =~ /^\d+(?:\.\d+)?$/ ) ) {
                    &Error(
"Fonts attribute '$attribute' invalid. Specify numeric value."
                    );
                    &GetData;
                    next FontData;
                }
        ...
        if ( @fontfiles{ lc($fontfile) } eq "" ) {
            $listfonts = $true;
            &Error(
"Section Fonts invalid. Attribute 'font': file '$fontfile.ttf' not available."
            );
            &GetData;
            next Fonts;
        }
...
"PlotData invalid. No (valid) command 'TimeAxis' specified in previous lines."
  ...
  PlotData:
    while ( ( !$InputParsed ) && ( !$NoData ) ) {
        ...
        if ($fonts_specified
          )    # set default when external fonts are used in input
        {
...
#  if (defined (@PlotDefs {"barset"}))    { $barset    = @PlotDefs {"barset"} ; }
        ...
        if ( defined( @PlotDefs{"anchor"} ) ) {
            $anchor = @PlotDefs{"anchor"};
        }
        if ( defined( @PlotDefs{"align"} ) ) { $align = @PlotDefs{"align"}; }
        if ( defined( @PlotDefs{"shiftx"} ) ) {
            $shiftx = @PlotDefs{"shiftx"};
        }
        if ( defined( @PlotDefs{"shifty"} ) ) {
            $shifty = @PlotDefs{"shifty"};
        }
...
"PlotData invalid. Attribute 'mark': unknown color '$markcolor'.\n"
                ...
                if ( !defined( @Fonts{ lc($attrvalue) } ) ) {
                    &Error(
"PlotData invalid. Attribute '$attribute' contains unknown font id '$attrvalue'.\n"
...
#        }
        ...
        if ( &BarDefined( $bar . "#1" ) )    # bar is actually a bar set
        {
                ...
                || ( $text eq " " ) )        # data line ?
            {
                ...
                if (   ( @BarSetMaxBars{$bar} > 0 )
                    && ( $barndx > @BarSetMaxBars{$bar} )
                  )                          # auto break on max bars reached
                {
                    $barndx = 1;
                }
...
sub ParseTextData {
    ...
    my ( $pos, $posexplicit, $tabs, $fontsize, $lineheight, $lineheightexplicit,
        $textcolor, $text, $link, $hint, $font );
...
"TextData invalid. Attribute '$attribute' contains unknown color '$attrvalue'.\n"
                ...
                if ( !defined( @Fonts{ lc($attrvalue) } ) ) {
                    &Error(
"TextData invalid. Attribute '$attribute' contains unknown font id '$attrvalue'.\n"
                          . "  Specify in command 'Fonts' before this command."
                    );
                    &GetData;
                    next TextData;
...
# substract fontsize / 100  (= char height in pixels) from start height -> text starts below posy
        if ( $fontsize > 0 ) { $fontsize2 = $fontsize / 100; }
        else {
        ...
        if ( $fontsize > 0 ) { $posy -= 0.18 * $fontsize2 + 0.03; }
        else {
...
sub WriteProcAnnotate {
    ...
    if ( $font ne "" ) {
        &WriteProcAnnotateFreeType(
            $bar,  $shiftx,    $xpos,     $ypos,
            $text, $textcolor, $fontsize, $align,
            $link, $hint,      $font
        );
        $freetype = $true;
    }
...
sub WriteProcAnnotateFreeType {
    ...
    if ( $align =~ /left/i ) {
        &WriteProcAnnotateSegment(
            $part1, $font, $fontsize, $align, $textcolor, $shiftx,
            $xpos,  $ypos, '',        '',     $hint,      '+',
            ''
        );
        &WriteProcAnnotateSegment(
            $part2,  $font, $fontsize, $align, $textcolor,
            $shiftx, $xpos, $ypos,     $bar,   $link,
            $hint,   '+',   $part1
        );
        &WriteProcAnnotateSegment(
            $part3,     $font,   $fontsize, $align,
            $textcolor, $shiftx, $xpos,     $ypos,
            '',         '',      $hint,     '+',
            $part1 . $part2
        );
    }
        ...
        &WriteProcAnnotateSegment(
            $part3, $font, $fontsize, $align, $textcolor, $shiftx,
            $xpos,  $ypos, '',        '',     $hint,      '-',
            ''
        );
        &WriteProcAnnotateSegment(
        ...
        if ( $part2 . $part3 eq "" ) {
            ...
            &WriteProcAnnotateSegment(
                $part2, $font, $fontsize, $align, $textcolor, $shiftx,
                $xpos,  $ypos, $bar,      $link,  $hint,      '',
                ''
            );
        }
        else {
            &WriteProcAnnotateSegment( $part1, $font, $fontsize, $align,
                $textcolor, $shiftx, $xpos, $ypos, '', '', $hint, '+', '',
                $part1 . $part2 . $part3 );
            &WriteProcAnnotateSegment(
                $part2,     $font,
                $fontsize,  $align,
...
sub WriteProcAnnotateSegment {
    ...
    if ( $parts123 ne ""
      ) # start centered texts half of full text width to the left, than treat as left aligned
    {
        $align      = 'left';
        $macroshift = $true;
        push @PlotTextsPng,
          "#set XSHIFT = \$textwidth(\"$parts123\", \"$font\", $fontsize)\n";
    ...
    if ( $partsdone ne "" ) {
        if ( !$macroshift ) {
            push @PlotTextsPng,
"#set XSHIFT = \$textwidth(\"$partsdone\", \"$font\", $fontsize)\n";
        }
        else {
            push @PlotTextsPng,
"#set DXSHIFT = \$textwidth(\"$partsdone\", \"$font\", $fontsize)\n";
    ...
    my $textdetails =
      "  textdetails: align=$align size=$fontsize color=$textcolor font=$font";
    if ( $link ne "" ) {
        $textdetails =
"  textdetails: align=$align size=$fontsize color=$LinkColor font=$font";
...
"Links on right aligned texts are only supported for svg output,\npending Ploticus bug fix."
    ...
    if   ( $part =~ /^\s/ ) { push @PlotTextsPng, "  text: \n\\$part\n\n"; }
    else                    { push @PlotTextsPng, "  text: $part\n\n"; }
}
...
sub WriteText {
    ...
    foreach $text (@Text) {
        if ( $text !~ /^[\n\s]*$/ ) {
            ...
            $hint2 = "";
            ( $text, $link2, $hint2 ) =
              &ProcessWikiLink( $text, $link2, $hint2 );
...
# 2B = $MaxBarWidth
    ...
    else { $script .= "  " . @Axis{"time"} . "scaletype: date $DateFormat\n"; }
 ...
 #  $script .= "  clickmapurl: http://www.wikipedia.org/wiki/Vladimir_Lenin\n" ;
    ...
    foreach $color ( sort keys %Colors ) {
        ...
        if (   ( defined( @ColorLabels{$color} ) )
            && ( @ColorLabels{$color} ne "" ) )
        {
...
#   $script .= "  clickmapurl: http://www.wikipedia.org/wiki/Vladimir_Lenin\n" ;
    ...
    if ( $#Bars > 0 ) {
        ...
        foreach $bar (@Bars2) {
            ...
            if ( $text =~ /\[.*\]/
              ) # text contains link ? do not write in black, ugly with freetype antialiased font
...
# my $cmd = "$pl $map -" . "svg" . " -o $file_vector $file_script -tightcrop" ;
    ...
    if ( $linkmap && $showmap ) { $map .= " -csmapdemo"; }
...
# $cmd = "$pl $map -" . $fmt . " -o $file_bitmap $file_script -tightcrop -diagfile $file_pl_info -errfile $file_pl_err" ;
    ...
    if ($fonts_specified
      )    # tell Ploticus to use freetype lib and set default font
    {
        $cmd .= " -font vera";
    }
    ...
    if   ( -e $file_bitmap ) { &Trace("Bitmap '$file_bitmap' created\n"); }
    else                     { &Trace("Bitmap '$file_bitmap' not found!\n"); }
...
"---------------------------------------------------------------------------\n";
    ...
    if ( $file_htmlmap ne "" ) {
        ...
        else { &Trace("No map file created!\n"); }
    }
      ...
      )    # correct click coordinates of right aligned texts (Ploticus bug)
    {
        ...
        foreach my $line (@map2) { &Trace($line); }
        &Trace("\n\n");
    }
...
sub ValidAttributes {
    ...
    if ( $command =~ /^BarData$/i ) {
        return (
            CheckAttributes( $command, "", "bar,barset,link,maxbars,text" ) );
    }
    ...
    if ( $command =~ /^PlotData$/i ) {
        return (
            CheckAttributes(
                $command,
                "",
"align,anchor,at,bar,barset,color,font,fontsize,from,link,mark,shift,text,textcolor,till,width"
            )
        );
    }
    ...
    if ( $command =~ /^TextData$/i ) {
        return (
            CheckAttributes(
                $command, "",
                "font,fontsize,lineheight,link,pos,tabs,text,textcolor"
            )
        );
    }
...
sub CheckPreset {
    ...
    if (   ( !$newcommand )
        && ( $command =~ /^(?:DrawLines|PlotData|TextData)$/i ) )
    {
        $addvalue = $false;
    }
    $prevcommand = $command;
...
sub NormalizeURL {
    my $url = shift;
    $url =~
      s/(https?)\:?\/?\/?/$1:\/\//;    # add possibly missing special characters
    $url =~ s/ /%20/g;
    return ($url);
}
...
sub ProcessWikiLink {
        ...
        "" )    # ignore wiki brackets in text when explicit link is specified
    {
        ...
        if (
            $text =~ /\[.+\]/ ) # keep first link in text segment, remove others
        {
            $link = $text;
            $link =~ s/\n//g;
            $link =~ s/^[^\[\]]*\[/[/x;
...
sub Abort {
    ...
    print FILE_OUT
      "<left><p>EasyTimeline $version<p><b>Timeline generation failed: "
      . &EncodeHtml($msg)
      . "</b><p></left>\n";
...
sub Trace {
    if ($trace) { print FILE_TRACE (shift); }
}
